...
{
    public partial class SchCounter : Form
    {
        List<Counter> list = new List<Counter>();
        List<Counter> Sublist = new List<Counter>();
        ...
        private void SchCounter_Load(object sender, EventArgs e)
        {
            list = new CategoryControlle.CounterController().GetCounter();
            Grd.DataSource = list;
            FormatGrid();
            LoadTempGrid();
        }
        ...
        private bool FilterBarFun(Counter c)
        {
            try
            {
                ...
                for (int i = 0; i < sr.Length; i++)
                {
                    ...
                    switch (fieldName)
                    {
                        case "[ID]":
                            ...
                            break;
                        case "[CounterName]":
                            res = res + " " + Convert.ToString((c.CounterName.ToString().ToUpper().StartsWith(fieldValue.ToString().ToUpper())));
                            break;
        ...
        private void FormatGrid()
        {
            try
            {
                ...
                Grd.Columns["CounterName"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                Grd.Columns["CounterName"].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleLeft;
                Grd.Columns["CounterName"].Width = 50;
                ...
                AccountsGlobals.ExtendCol(ref Grd, "CounterName");
        ...
        private void SetParaOut()
        {
            try
            {
                if (list.Count > 0)
                {
                    ...
                    counter = new CategoryControlle.CounterController().GetSingleCounter(Convert.ToInt32((Grd.Rows[RowIndex].Cells["ID"].Value.ToString())));
                }
                this.Close();
            }
            catch (Exception ex)
